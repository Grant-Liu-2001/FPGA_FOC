...
module foc_top #(
    // ----------------------------------------------- 模块参数 ---------------------------------------------------------------------------------------------------------------------------------------------------
    parameter              INIT_CYCLES  = 15000000, // 决定了初始化步骤占多少个时钟(clk)周期，取值范围为1~4294967294。该值不能太短，因为要留足够的时间让转子回归电角度=0。例如若时钟(clk)频率为 36.864MHz，INIT_CYCLES=16777216，则初始化时间为 16777216/36864000=0.45 秒
    parameter logic        ANGLE_INV    = 1'b0,     // 若角度传感器没装反（A->B->C->A 的旋转方向与 φ 增大的方向相同），则该参数应设为 0。若角度传感器装反了（A->B->C->A 的旋转方向与 φ 增大的方向相反），则该参数应设为 1。
    parameter logic [ 7:0] POLE_PAIR    = 8'd14,     // 电机极对数 (简记为N)，取值范围1~255，根据电机型号决定。（电角度ψ = 极对数N * 机械角度φ）
    ...
    parameter logic [23:0] Kp           = 24'd5169,// 电流环 PID 控制算法的 P 参数
    parameter logic [23:0] Ki           = 24'd3     // 电流环 PID 控制算法的 I 参数
) (
    // ----------------------------------------------- 驱动时钟和复位 ---------------------------------------------------------------------------------------------------------------------------------------------
...
always @ (posedge clk or negedge init_done)
    if(~init_done) begin
        ...
        en_iabc <= en_adc;
        if(en_adc) begin
            ia <= $signed( ({4'b0, adc_b} + {4'b0, adc_c} - {3'b0, adc_a, 1'b0})*24 );   // Ia = ADCb + ADCc - 2*ADCa
            ib <= $signed( ({4'b0, adc_a} + {4'b0, adc_c} - {3'b0, adc_b, 1'b0})*24 );   // Ib = ADCa + ADCc - 2*ADCb
            ic <= $signed( ({4'b0, adc_a} + {4'b0, adc_b} - {3'b0, adc_c, 1'b0})*24 );   // Ic = ADCa + ADCb - 2*ADCc
        end
    end
